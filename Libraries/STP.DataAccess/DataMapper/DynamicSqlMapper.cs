...
namespace STP.DataAccess.DataMapper
{
    /// <summary>
    ///     A light weight object mapper for ADO.NET
    /// </summary>
    public static class DynamicSqlMapper
    {
        /// <summary>
        ...
        /// <summary>
        ///     This is a micro-cache; suitable when the number of terms is controllable (a few hundred, for example), and strictly
        ///     append-only; you cannot change existing values. All key matches are on **REFERENCE** equality. The type is fully
        ///     thread-safe.
        /// </summary>
        ...
        {
            var couSTP = new Dictionary<int, int>();
            foreach (var key in _queryCache.Keys)
            {
                int count;
                if (!couSTP.TryGetValue(key.hashCode, out count))
                {
                    couSTP.Add(key.hashCode, 1);
                }
                else
                {
                    couSTP[key.hashCode] = count + 1;
                }
            }
            return from pair in couSTP
                where pair.Value > 1
    ...
    /// <returns>Number of rows affected</returns>
        ...
        /// </remarks>
        /// <returns>
        ///     A sequence of data of the supplied type; if a basic type (int, string, etc) is queried then the data from the
        ///     first column in assumed, otherwise an instance is created per row, and a direct column-name===member-name mapping
            ...
            IDataReader reader)
        {
            ...
            {
                var curreSTPplit = splits[splitIndex];
                ...
                if (type != typeof (Object))
                {
                    ...
                    {
                        if (string.Equals(name, curreSTPplit,
                ...
                for (pos = startingPos; pos < reader.FieldCount; pos++)
                {
                    ...
                    if (string.Equals(reader.GetName(pos), curreSTPplit,
                        StringComparison.OrdinalIgnoreCase))
                    {
                        if (skipFirst)
                        {
        ...
        public static Action<IDbCommand, object> CreateParamInfoGenerator(Identity identity)
        {
            ...
            foreach (var prop in props)
            {
                ...
                {
                    // this actually represeSTP special handling for list types;
                    ...
                    il.EmitCall(OpCodes.Call, typeof (DynamicSqlMapper).GetMethod("PackListParameters"), null);
                    // stack is [parameters]
                    continue;
                }
...
#pragma warning restore 618
            ...
            )
        {
            ...
            var setters = (
                ...
                let prop =
                    ...
                    properties.FirstOrDefault(
                        p => string.Equals(p.Name, n, StringComparison.OrdinalIgnoreCase))
            ...
            foreach (var item in setters)
            {
                if (item.Property != null || item.Field != null)
                {
                    ...
                    {
                        il.EmitCall(OpCodes.Call, typeof (DynamicSqlMapper).GetMethod(
            ...
            il.EmitCall(OpCodes.Call, typeof (DynamicSqlMapper).GetMethod("ThrowDataException"), null);
        ...
        public class GridReader : IDisposable
        {
                ...
                object func, string splitOn)
            {
                ...
                try
                {
                    foreach (
                        var r in
                            DynamicSqlMapper.MultiMapImpl<TFirst, TSecond, TThird, TFourth, TFifth, TReturn>(null, null,
                                func,
                                null, null,
                                splitOn, null,
                                null, reader,
    ...
    /// </summary>
    public class DynamicParameters : DynamicSqlMapper.IDynamicParameters
    {
        private static readonly Dictionary<DynamicSqlMapper.Identity, Action<IDbCommand, object>> paramReaderCache =
            new Dictionary<DynamicSqlMapper.Identity, Action<IDbCommand, object>>();
        ...
        void DynamicSqlMapper.IDynamicParameters.AddParameters(IDbCommand command, DynamicSqlMapper.Identity identity)
        {
            if (templates != null)
            {
                foreach (var template in templates)
                {
                    ...
                    lock (paramReaderCache)
                    {
                        ...
                        {
                            appender = DynamicSqlMapper.CreateParamInfoGenerator(newIdent);
        ...
        /// <summary>
        ///     Append a whole object full of params to the dynamic EG: AddParams(new {A = 1, B = 2}) // will add property A and B
        ///     to the dynamic
        /// </summary>
        /// <param name="param"> </param>
    ...
    /// <summary>
    ///     This class represeSTP a SQL string, it can be used if you need to denote your parameter is a Char vs VarChar vs
    ///     nVarChar vs nChar
    /// </summary>
