...
{
    public class SOAPoliceWorkflowService : ISOAPoliceWorkflowService
    {
        readonly IWorkflowService workflowService;

        public SOAPoliceWorkflowService()
        {
        ...
        {
            var workflow = workflowService.GetWorkflowByName(WorkflowProcessKey.Process_SOAWorkflowNotificationsApproval.ToString());
        ...
        public WorkflowProcessDetails StartProcess(string esdalReferenceNumber, bool setApplicationKey = false, string organizationId = null, decimal esdalKey = 0)
        {
            ...
            {
                return StartSOAProcess(esdalReferenceNumber, workflow, setApplicationKey, organizationId, esdalKey);
            }
        ...
        private WorkflowProcessDetails StartSOAProcess(string esdalReferenceNumber, WorkflowModel workflow, bool setApplicationKey = false, string organizationId = null, decimal esdalKey = 0)
        {
            dynamic startProcessApplicationPayload = new ExpandoObject();
            startProcessApplicationPayload.decideNextTask = false;
            var workflowResponse = workflowService.StartProcess(WorkflowProcessKey.Process_SOAWorkflowNotificationsApproval,
                                                                payload: startProcessApplicationPayload,
                                                                applicationId: setApplicationKey ? esdalReferenceNumber : null,
                                                                orgId: organizationId,
                                                                subjectId: WorkflowActivityConstants.Wf_Subject_SOAPoliceNotificationProcess);
