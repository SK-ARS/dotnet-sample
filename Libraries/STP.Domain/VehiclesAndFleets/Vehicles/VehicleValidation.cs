...
namespace STP.Domain.VehiclesAndFleets.Vehicles
{
    public class VehicleValidation
    {
        ...
        public VehicleImportModel ValidateVehicleData(VehicleImportJson vehicle)
        {
            ...
            {
                var totalCount = vehicle.VehicleComponents.Count;
                for (int i = 0; i < vehicle.VehicleComponents.Count; i++)
                {
                    VehicleComponentDetails vehicleComponentDetails = ValidateComponentData(vehicle.VehicleComponents[i], vehicleConfigDetails.MovementClassification,i, totalCount);
        ...
        public VehicleConfigDetails ValidateVehicleConfigData(VehicleConfiguration vehicleConfiguration)
        {
            ...
            #region Width
            if (vehicleConfiguration.Width >= 0)
            {
                if ((movementClassification == 270006 || movementClassification == 270007) && vehicleType == 244007 && (vehicleConfiguration.Width < 5 || vehicleConfiguration.Width > 200))
                {
                    errorCount++;
                    ErrorMessage = ErrorMessage + errorCount + ". Overall width should be in between 5 and 200~";
                    Logger.GetInstance().LogMessage(Log_Priority.FATAL_ERROR, LogInstance + @" - ValidateVehicleConfigData : {0}", "Error : Overall width should be in between 5 and 200");
                }               
            ...
            if (vehicleConfiguration.MaxAxleWeight >= 0)
            {
            ...
            #region Wheel Base
            if (vehicleConfiguration.WheelBase >= 0)
            {
            ...
            #region Speed
            if (vehicleConfiguration.Speed >= 0)
            {
            ...
            #region Tyre Spacing
            if (vehicleConfiguration.TyreSpacing >= 0)
            {
            ...
            if(vehicleConfiguration.VehicleRegistration != null && vehicleConfiguration.VehicleRegistration.Any())
            {
                foreach(var item in vehicleConfiguration.VehicleRegistration)
                {
                    ...
                    {
                        errorCount++;
                        ErrorMessage = ErrorMessage + errorCount + ". Invalid Serial Number~";
                        Logger.GetInstance().LogMessage(Log_Priority.FATAL_ERROR, "ValidateVehicleConfigData : {0}", "Error : Invalid Serial Number");
                    }
                    if (string.IsNullOrWhiteSpace(item.Registration))
                    {
                        errorCount++;
                        ErrorMessage = ErrorMessage + errorCount + ". Invalid Registration~";
                        Logger.GetInstance().LogMessage(Log_Priority.FATAL_ERROR, "ValidateVehicleConfigData : {0}", "Error : Invalid Registration");
                    }
                }
            }
            else
            {
                errorCount++;
                ErrorMessage = ErrorMessage + errorCount + ". Vehicle registration is required ~";
                Logger.GetInstance().LogMessage(Log_Priority.FATAL_ERROR, "ValidateVehicleConfigData : {0}", "Error : Vehicle registration is blank ");
            }
        ...
        public VehicleComponentDetails ValidateComponentData(VehicleComponent vehicleComponents, int movementClassification, int current, int totalCount)
        {
            int errorCount = 0;
            int componentType = 0;
            ...
            if (vehicleComponents.ReducibleGroundClearance >= 0)
            {
                bool failed = false;
            ...
            if (vehicleComponents.LeftOverhang >= 0)
            {
                bool failed = false;
            ...
            if (vehicleComponents.RightOverhang >= 0)
            {
                bool failed = false;
                if (componentType == 234006)
                {
            ...
            if (vehicleComponents.FrontOverhang >= 0)
            {
                bool failed = false;
            ...
            if (vehicleComponents.RearOverhang >= 0)
            {
                bool failed = false;
            ...
            {
                if (vehicleComponents.AxleSpacingToFollowing >= 0)
                {
            ...
            #region Description
            if (vehicleComponents.Notes.Length > 200)
            {
                errorCount++;
            ...
            {
                vehicleComponentDetails.Description = vehicleComponents.Notes;
            }
            #endregion
            ...
            else
            {
                ...
                if (vehicleComponents.Axles != null)
                {
                    ...
                    {
                        if (item.AxleNumber <= 0)
                        {
                            errorCount++;
                            ErrorMessage = ErrorMessage + errorCount + ". Invalid Axle Number~";
                            Logger.GetInstance().LogMessage(Log_Priority.FATAL_ERROR, "ValidateVehicleConfigData : {0}", "Error : Invalid Axle Number");
                        }
                        if (item.NoOfWheels <= 0)
                        {
                            errorCount++;
                            ErrorMessage = ErrorMessage + errorCount + ". Invalid No Of Wheels~";
                            Logger.GetInstance().LogMessage(Log_Priority.FATAL_ERROR, "ValidateVehicleConfigData : {0}", "Error : Invalid No Of Wheels");
                        }
                        if (item.AxleWeight <= 0)
                        {
                            errorCount++;
                            ErrorMessage = ErrorMessage + errorCount + ". Invalid Axle Weight~";
                            Logger.GetInstance().LogMessage(Log_Priority.FATAL_ERROR, "ValidateVehicleConfigData : {0}", "Error : Invalid Axle Weight");
                        }
                        if (currentAxleIndex < totalAxleCount)
                        {
                            ...
                            {
                                errorCount++;
                                ErrorMessage = ErrorMessage + errorCount + ". Invalid Distance To Next Axle~";
                                Logger.GetInstance().LogMessage(Log_Priority.FATAL_ERROR, "ValidateVehicleConfigData : {0}", "Error : Invalid Distance To Next Axle");
                            }
                        }
