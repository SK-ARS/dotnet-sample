    ...
    $(document).ready(function () {
        $("#btn_cancel").on('click', BacktorouteList);
        $("#btn_hualierdetails").on('click', view_hualierdetails);
        if ('@Model.Count' <= 1) {
            $("#sort").hide();
        }
        var status = '@Model[0].NENRouteStatus';
        if ('@Model.Count' == 1) {
        ...
        $('#btn_cancel').hide();
        if ($("#hIs_NEN").val() == "true") {
            $("#hNEN_RouteStatus").val('@ViewBag.NENrouteStatus');
            $('#sort').hide();
            $('#hNENRoute_Id').val('@ViewBag.hNENRoute_Id');
    ...
    function EditRouteApplication(RouteId, RouteName, RouteType) {
        ...
        if ($("#hIs_NEN").val()) {
            view_hualierdetails();  // automatically calls the haulier description when a nen route is edited
        }
            ...
            //editeRoute(RouteId, RouteType);
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        ...
                        else if (flag == 2 && '@Session["RouteFlag"]' == 2) {
                        ...
                        else if (flag == 1 && '@Session["RouteFlag"]' == 2) {
    ...
    function editCandidatRoute(RouteId, RouteType, RouteName) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                else if (flag == 2 && '@Session["RouteFlag"]' == 4) {
                ...
                else if (flag == 1 && '@Session["RouteFlag"]' == 4) {
   ...
   // displaySoRouteDescMap(RouteId, RouteType);
    $('#leftpanel').html('');
    var url = '@Url.Action("A2BLeftPanel", "Routes", new { routeID = "_ID" })';
        $.ajax({
    ...
    function displaySoRouteDescMap(RouteId, RouteType) {
                ...
                if (flag == 1 || '@Session["RouteFlag"]' == 1 || '@Session["RouteFlag"]' == 3) {
                    ...
                    if (result.result.routePartDetails.routeDescr != null && result.result.routePartDetails.routeDescr != "") {
                        ...
                        if (StrDes.length > 149) {
                            $("#RouteDesc").html(StrDes.substring(0, 149)+"...");
                            document.getElementById('RouteDesc').title = StrDes;
                        }
                        else $("#RouteDesc").html(StrDes);
                    }
                    var LI = "";
                    for (var i = 0 ; i < count; i++) {
        ...
        if ($("#hIs_NEN").val() == "true") {
            ...
            if (RouteId == $('#hreturnLeg_routeID').val())
                ...
                ActionForReturnRout = 0;
            @*  $.ajax({
                url: "../NENNotification/SaveAuditLogs",
                type: 'GET',
                //contentType: "application/json; charset=utf-8",
                data: { VAuditActionType: '@AuditActionType.soauser_view_nen_route_details' },
                success: function () {
                },
                error: function () {
                },
    ...
    function DeleteSelectedRoute(_this, routeId, RouteType)
    {
        ...
        G_Route_flag = '@Session["RouteFlag"]';
        ////
    ...
    function OnDeleteShow_Warning()
    {
        ...
        $.ajax({
            ...
            cache: false,
            async: false,
            data: { RouteID: routeId, RouteType: RouteType, RouteFlag: '@Session["RouteFlag"]'},
            beforeSend: function () {
            },
    ...
    {
        if ('@Session["RouteFlag"]' == 3)//notification
            Delete_Noti_Route();
        else if ('@Session["RouteFlag"]' == "4")
    ...
    function ViewRouteOnMapNEN(RouteId, RouteType, rtStatus, hideRoute = false) {
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                $("#RouteName").html(rtDetails.routeName);
                ...
                $("#appRouteMap").show();
                $("#appRouteMap").html('');
                $("#appRouteMap").load('@Url.Action("A2BPlanning", "Routes", new { routeID = 0 })', function () {
