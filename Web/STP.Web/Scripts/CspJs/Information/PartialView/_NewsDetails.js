...
if($('#hf_mode').val() ==  'Edit')
    $('#EndDate').val(hf_DateTimeToday);
        ...
        $("#btnback").click(RedirectToMainPage);
        $(".hideerr").click(hideError);
        $(".hideerr5").click(hideError);
    ...
    $.ajax({
        ...
        dataType: "json",
        data: '{ contentId: "' + currentContentID + '", fileType: "' + fileType + '", fileName: "' + associatedFile + '" }',
        processdata: true,
...
function validation() {
    ...
    $("#Title").keyup(function () {
        ...
        if (PageNameValid == "link" || PageNameValid == "download") {
            if ($('#Title').val() == '') {
                ...
                validationForTitle.innerHTML = "Minimum 10, maximum 100 characters";
                validationForTitle.style.display = 'block';
            }
    ...
    $(".custom-file-input3").on("change", function () {
        let fileName = $(this).val().split("\\").pop();
        $(this).siblings(".custom-file-label3").addClass("selected").html(fileName);
    });
    $(".custom-file-input4").on("change", function () {
        let fileName = $(this).val().split("\\").pop();
        $(this).siblings(".custom-file-label4").addClass("selected").html(fileName);
    });
