    ...
    $(document).ready(function () {
        ...
        $("#MoveStat").val('@ViewBag.VersionStatus');
        DisplayGeneralDetails();
    });

    function DisplayGeneralDetails() {
        SetTabStyle(1);
        LoadContentForAjaxCalls("POST", '../Notification/DisplayGeneralDetails', { notificationId: $("#NotificationId").val(), isHistory: $("#Historical").val() }, '#general_details');
    }

    function DisplayRouteVehicle() {
        SetTabStyle(2);
        LoadContentForAjaxCalls("POST", '../Notification/DisplayRouteVehicle', { notificationId: $("#NotificationId").val(), isHistory: $("#Historical").val() }, '#route_vehicle_details');
    }

    function DisplayRouteAssessment() {
        SetTabStyle(3);
        LoadContentForAjaxCalls("POST", '../Application/RouteAnalysisPanel', { analysisId: $('#NotifAnalysisId').val(), contentRefNo: $('#CRNo').val() }, '#route_assessment_details');
    }
    ...
    function LoadContentForAjaxCalls(Type, Url, Params, ResLoadContnr) {
        $.ajax({
            type: Type,
            url: Url,
            data: Params,
