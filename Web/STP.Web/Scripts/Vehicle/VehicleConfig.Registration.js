...
function DeleteRegistrationConfig1() {
    ...
    var msg = 'Do you want to delete ?';
    ShowWarningPopupParam(msg, 'DeleteRegistrationConfig', '', IdNum1);
    //DeleteRegistrationConfig(IdNum1, x);
    //$('#pop-warning').hide();
}
...
function AddRegistrationConfig(_this) {
    ...
    if (registrationParams.length > 0) {
        $.ajax({
            url: '../VehicleConfig/SaveRegistrationID',
            data: '{"configId":' + configId + ',"registrationParams":' + JSON.stringify(registrationParams) + '}',
            type: 'POST',
...
function DeleteRegistrationConfig( IdNum) {
