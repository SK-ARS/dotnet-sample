$(function () {
    ...
    $('body').on("keypress","#txt_search", function (event) {
            if (event.which == 13) {
    ...
    $('body').on('click', '.btn-fleetgrid-import-data', function (e) {
        var id = $(this).data('id');
        ImportData(id);
    });
    $('body').on('click', '.btn-fleetgrid-view-config-summary', function (e) {
        ViewConfigSummary();
    });
});
...
function ViewComponent(id) {
    $.ajax({
        ...
        complete: function () {            
            ...
            $('#div_view_component').append('<div><button class="next btngrad btnrds btnbdr btn-fleetgrid-import-data" aria-hidden="true" data-icon="&#xe0f4;" type="button" data-id=' + id+'">Import</button><button class="next btngrad btnrds btnbdr btn-fleetgrid-view-config-summary" aria-hidden="true" data-icon="&#xe119;" type="button">Back</button></div>')
