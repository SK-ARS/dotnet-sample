...
var plannedRouteIdVal = $('#hf_plannedRouteId').val();
var IsTextualRouteTypeVal = $('#hf_IsTextualRouteType').val();
var modelRoutePartVal = $('#hf_modelRoutePart').val();
var rpVal = $('#hf_rp').val();

var modelRoutePart = modelRoutePartVal;
...
$('body').on('click','#span-Edit_help', function(e) { 
  ...
  EditHelp_popup(this);
    ...
    if ($('#hf_IsStartAndEndPointOnly').val() != "True" && IsTextualRouteTypeVal != "True") {
    ...
    if (textrouteflag == true && RouteFlagVal != 2) {
...
$('#outline_btn_save').click(function () {
    if (rpObj != null) {
        if (modelRoutePart == undefined)
            modelRoutePart = rpObj;
        modelRoutePart.routePartDetails.routeID = plannedRouteIdVal;
    ...
    if ($('#PortalType').val() == '696008') {
        try {
            if ($("#chkdockcaution").is(':checked')) {
                modelRoutePart.routePartDetails.dockcaution = "True";
            }
        }
        catch (err) { }
    }
    ...
    if ($('#hf_IsStartAndEndPointOnly').val() != "True") {
        ...
        var compressedRoutePart = pako.gzip(JSON.stringify({ RoutePart: modelRoutePart }));
        ...
        $.ajax({
            ...
            data: JSON.stringify({ compressedRoutePart: base64RoutePartData, PlannedRouteId: id, orgID: 1101 }),
...
function ClosePopUp12() {
    if (RouteFlagVal == 2 || RouteFlagVal == 1) {
        $("#dialogue").hide();
        $("#pop-warning").hide();
        $("#overlay").hide();
            ...
            setRoutePoint(endDesc, 1, endNorthing, endEasting, 1, function (result) {
                if (result == true) {
                    ...
                    $.ajax({
                        ...
                        async: false,
                        contentType: 'application/json; charset=utf-8',
                        data: JSON.stringify({ compressedRoutePart: base64RoutePartData, PlannedRouteId: 0, RouteFlag: 3 }),
                        success: function (val) {
                            var rt = val.value;
                            stopAnimation();
                            RN = "Route " + "'" + RN + "' " + "saved successfully";
