...
(function ($) {
	...
	Redactor.prototype = {
		...
		restoreSelection: function()
		{
			...
			{
				if (this.opts.iframe === false && $(this.savedSel[0]).closest('.redactor_editor').size && $(this.savedSel[0]).closest('.redactor_editor').size() == 0)
				{
					this.$editor.focus();
				}
				else
		...
		//function for saving data as xml
		SaveNotes2Haulier: function () {
			var data = $('.redactor_editor').html();
			data = this.stripTags(data);
			this.restoreSelection();
			this.execCommand('inserthtml', data);
			this.modalClose();
