    ...
    <html>
      ...
      <body style="font-family:Arial">
          ...
          <tr>
            ...
            <td colspan="3">
              ...
              <xsl:text disable-output-escaping="yes"><![CDATA[&nbsp;]]></xsl:text><xsl:call-template name="parseString">
                <xsl:with-param name="list" select="ns1:JourneyFromToSummary/ns2:From"/>
              </xsl:call-template> to
              <xsl:call-template name="parseString">
                <xsl:with-param name="list" select="ns1:JourneyFromToSummary/ns2:To"/>
              </xsl:call-template>
            </td>
          </tr>
          <tr>
        ...
        <xsl:variable name="PlateNoString">
          <xsl:choose>
            <xsl:when test="contains(ns3:VehicleSummary/ns3:ConfigurationIdentityListPosition/ns3:ConfigurationIdentity/ns3:PlateNo, '##**##') or  contains(ns3:VehicleSummary/ns3:ConfigurationIdentityListPosition/ns3:ConfigurationIdentity/ns3:FleetNo, '##**##')">
              <xsl:for-each select="ns1:RouteParts/ns2:RoutePartListPosition/ns2:RoutePart/ns2:RoadPart/ns2:Vehicles/ns3:VehicleSummaryListPosition/ns3:VehicleSummary/ns3:ConfigurationIdentityListPosition">
                <xsl:choose>
                  <xsl:when test="ns3:ConfigurationIdentity/ns3:PlateNo!='' and  string-length(ns3:ConfigurationIdentity/ns3:PlateNo) &gt; 0">
                    <strike>
                      or <xsl:value-of select="substring-before(ns3:ConfigurationIdentity/ns3:PlateNo, '##**##')"/>
                    </strike>
                    <xsl:text disable-output-escaping="yes"><![CDATA[&nbsp;]]></xsl:text>
                    <xsl:value-of select="substring-after(ns3:ConfigurationIdentity/ns3:PlateNo, '##**##')"/>
                  </xsl:when>
                  <xsl:otherwise>
                    <strike>
                      or <xsl:value-of select="substring-before(ns3:ConfigurationIdentity/ns3:FleetNo, '##**##')"/>
                    </strike>
                    <xsl:text disable-output-escaping="yes"><![CDATA[&nbsp;]]></xsl:text>
                    <xsl:value-of select="substring-after(ns3:ConfigurationIdentity/ns3:FleetNo, '##**##')"/>
                  </xsl:otherwise>
                </xsl:choose>
              </xsl:for-each>
            </xsl:when>
        ...
        <p>
          <xsl:choose>
            <xsl:when test="$DocType = 'PDF'">
              <b>
                AFFECTED STRUCTURES (<xsl:if test="ns1:RouteParts/ns2:RoutePartListPosition/ns2:RoutePart/ns2:Name != ''">
                  <xsl:call-template name="parseString">
                    <xsl:with-param name="list" select="ns1:RouteParts/ns2:RoutePartListPosition/ns2:RoutePart/ns2:Name"/>
                  </xsl:call-template>
                </xsl:if>)
              </b>
              <br/>
        ...
        <xsl:if test="ns1:IsIndemnityConfirmed = 'true'">
          <table border = "0" width = "100%" >
            ...
            <tr>
              ...
              <td colspan="68" >
                We<xsl:text disable-output-escaping="yes"><![CDATA[&nbsp;]]></xsl:text>
                <xsl:call-template name="parseString">
                  <xsl:with-param name="list" select="ns1:HaulierDetails/ns2:HaulierName"/>
                </xsl:call-template>
                ...
                <xsl:if test="ns1:OnBehalfOf">
                  ...
                  <xsl:if test="contains(ns1:OnBehalfOf, '##**##')">
                    <strike>
                      (on behalf of <xsl:value-of select="substring-before(ns1:OnBehalfOf, '##**##')"/>)
                    </strike>
                    <xsl:text disable-output-escaping="yes"><![CDATA[&nbsp;]]></xsl:text>
                    <b>
                      <u>
