...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:a="http://www.esdal.com/schemas/core/routeanalysis" xmlns:b="http://www.esdal.com/schemas/core/route" xmlns:c="http://www.esdal.com/schemas/core/esdalcommontypes">
            ...
            <div id="dlfId" class="flexer radioSOA mt-4 ml-0">
              <xsl:for-each select="/a:AnalysedRoadsRoute/a:AnalysedRoadsPart">
                ...
                <div class="form-check">
                  <input class="form-check-input road_select" type="radio" myaffroad ="1" name="flexRadioDefault" id="flexRadioDefault11" value="{$roaddiv}"/>
          ...
          <xsl:for-each select="/a:AnalysedRoadsRoute/a:AnalysedRoadsPart">
            ...
            <div id="{$roaddiv}" class="roaddivclass" >
              <table>
                <thead>
                  <tr>
                    <th style="padding-left: 2rem !important;">Road</th>
                    <th>Distance</th>
                  </tr>
                </thead>
                <tbody>
                  <xsl:for-each select="a:SubPart/a:Roads/a:Path/a:RoadsPathSegment">
                    <xsl:if test="./a:Road/a:RoadResponsibility/a:RoadResponsibleParty/@OrganisationId = $OrgId">
                      <xsl:if test="./a:Road/a:RoadIdentity/c:Number">
                        <tr>
                          <td class="text-color1" style="padding-left: 2rem !important;">
  ...
  <xsl:template match="a:Distance">
    <xsl:if test="./text() &lt; 1610">
      <xsl:value-of select="ceiling(./text() * 1.09361)"/> yards
    </xsl:if>
    <xsl:if test="./text() &gt; 1610 or ./text() = 1610">
      <xsl:value-of select="format-number(./text() * 0.000621371,'#.#')"/> miles
    </xsl:if>
  </xsl:template>
