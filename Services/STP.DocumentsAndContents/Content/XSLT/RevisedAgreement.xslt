  ...
  <xsl:template match="/ns1:AgreedRoute">
    ...
    <html>
      <body style="font-family:Arial;font-size:13px;">
                    ...
                    <xsl:if test="contains(ns2:SentDateTime, '##**##')">
                      <xsl:element name="newdate">
                        <xsl:call-template name="FormatDate">
                          <xsl:with-param name="DateTime" select="substring-before(ns2:SentDateTime, '##**##')"/>
                          <xsl:with-param name="DateTime1" select="substring-before(ns2:SentDateTime, '##**##')"/>
                        </xsl:call-template>
                      </xsl:element>
                    </xsl:if>
          ...
          <tr>
            ...
            <td  align ="left" width ="60">
              <xsl:if test="ns2:HaulierDetails/ns2:HaulierContact != ''">
                <div>
                  <xsl:choose>
                    <xsl:when test="contains(ns2:HaulierDetails/ns2:HaulierContact, '##**##')">
                      <strike>
                        <xsl:value-of select="substring-before(ns2:HaulierDetails/ns2:HaulierContact, '##**##')"/>
                      </strike>
                      <b>
                        <u>
                          <xsl:value-of select="substring-after(ns2:HaulierDetails/ns2:HaulierContact, '##**##')"/>
                        </u>
                      </b>
                    </xsl:when>
                    <xsl:otherwise>
        ...
        <!-- If condition to check for non semi vehicles ends here-->
        <br/>
    ...
    <xsl:if test="contains($list, '##**##')">
      <strike>
        ...
        <xsl:if test="$UnitType=692002">
          <xsl:call-template name="ConvertToFeet">
            <xsl:with-param name="pText" select="substring-before($list, '##**##')"/>
          </xsl:call-template>
        </xsl:if>
      ...
      <b>
        <u>
          ...
          <xsl:if test="$UnitType=692002">
            <xsl:call-template name="ConvertToFeet">
              <xsl:with-param name="pText" select="substring-after($list, '##**##')"/>
            </xsl:call-template>
          </xsl:if>
