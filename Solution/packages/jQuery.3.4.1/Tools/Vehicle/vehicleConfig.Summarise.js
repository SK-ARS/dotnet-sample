...
function AddImagesAtTop() {
	...
	for (var i = 0; i < totlength; i++) {
		...
		if (i == 0) {
			$('#div_img_wrap').append('<div class="centre"  title="' + vehicleCompList[vehicleCompNum][i].ComponentName + '" ><div class="div_img_line vehiclecomp_active" id="div_' + i + '" compId="' + vehicleCompList[vehicleCompNum][i].ComponentTypeId + '" compTypeName="' + vehicleCompList[vehicleCompNum][i].ComponentName + '" number="' + i + '" latPos="' + latPosition + '" longPos="' + longPosition + '"><div class="vehiclecomp_div cursor" ><div class="vehiclecomponent ' + vehicleCompList[vehicleCompNum][i].ImageName + '" onclick="NavigateToSpecComponent(' + i + ')" title="' + title + '"></div></div></div></div>');
		}
		else {
			$('#div_img_wrap').append('<div class="centre"  title="' + vehicleCompList[vehicleCompNum][i].ComponentName + '" ><div class="div_img_line" id="div_' + i + '" compId="' + vehicleCompList[vehicleCompNum][i].ComponentTypeId + '" compTypeName="' + vehicleCompList[vehicleCompNum][i].ComponentName + '" number="' + i + '" latPos="' + latPosition + '" longPos="' + longPosition + '"><div class="vehiclecomp_div cursor" ><div class="vehiclecomponent ' + vehicleCompList[vehicleCompNum][i].ImageName + '" onclick="NavigateToSpecComponent(' + i + ')" title="' + title + '" ></div></div></div></div>');
		}
...
////function Load Appl Comp Grid
//function LoadApplicationGrid(compType) {
//    
//    //return false;
//    $.ajax({
//        url: "../VehicleConfig/FleetComponentList",
...
function AjaxImportComponent(vehicleConfigId, compId, vhclTypeId, latPosition, longPosition) {
	$.ajax({
...
//function AjaxAppVehicleImportComponent(vehicleConfigId, compId, vhclTypeId, latPosition, longPosition) {
 
//    $.ajax({
//        url: "../VehicleConfig/GetAppCompVehicleDescription",
//        type: 'POST',
//        cache: false,
//        async: false,
//        data: { vehicleId: vehicleConfigId, latitudePos: latPosition, longtitudePos: longPosition },
//        success: function (result) {
//            if (result.Success) {
//                //method
//                AjaxGetappVehComponentDesc();
//            }
//            else {
//               
//            }
...
//function AjaxGetappVehComponentDesc() {
...
//        type: 'POST',
//        cache: false,
//        data: { vehicleId: configId, latitudePos: latitudePos, longtitudePos: longtitudePos },
//        success: function (result) {
...
//            }
//            else {
//                DisplaySelectedComponent(result.desc.ComponentDescription, result.compId, result.ComponentName);
...
function DisplaySelectedComponent(desc, compId, compName) {
    selComp = compId;
	$('#div_tbl_grid').html('<table class="tbl_registration"><tr><th class="headgrad">Description</th><th class="headgrad"></th></tr><tr><td>' + desc + '</td><td><button class="tdbutton tdright btngrad btnrds btnbdr btnautowidth" aria-hidden="true" data-icon="&#xe08e;" type="button" onclick="DeleteCompFromConfig()">Replace</button><button class="tdbutton tdright btngrad btnrds btnbdr" aria-hidden="true" data-icon="&#xe000;" type="button" onclick="EditComponent(' + compId + ')">Edit</button><button class="tdbuttonauto tdright btngrad btnrds btnbdr" aria-hidden="true" type="button" onclick="AddToFleet(' + compId + ',\'' + compName + '\')">Add to Fleet</button></td></tr></table>');
}
...
function AddToFleet(componentId, compName) {
...
function CheckFormalNameExists(componentId, compName) {
	...
	$.ajax({
		...
		success: function (data) {
			compId = componentId;
			if (data.success > 0) {
				showWarningPopDialog('Component already exists. Do you want to over write?', 'No', 'Yes', 'CloseModelPop', 'AddComponentToFleet', 1, 'warning');
			}
			else {
				AddComponentToFleet();
			}
...
function AddComponentToFleet() {
	...
	$.ajax({
		...
		cache: false,
		async: false,
		data: { componentid: compId, NotificationID: notifid, vehicleConfigId: configId },
		beforeSend: function () {
			$('#pop-warning').hide();
		},
		success: function (result) {
...
function AddAppComponentToFleet() {
	...
	$.ajax({
		...
		cache: false,
		async: false,
		data: { componentid: compAppId, vehicleConfigId: configId, isAppRevId: ApplicationRevId, isApplication: true, flag: flag, isVR1: false },
		beforeSend: function () {
			$('#pop-warning').hide();
		},
		success: function (result) {
...
function AddVR1ComponentToFleet() {
	...
	$.ajax({
		...
		cache: false,
		async: false,
		data: { componentid: compVR1Id, vehicleConfigId: configId, isApplication: true, isAppRevId: ApplicationRevId, flag: flag, isVR1: true },
		beforeSend: function () {
			$('#pop-warning').hide();
		},
		success: function (result) {
